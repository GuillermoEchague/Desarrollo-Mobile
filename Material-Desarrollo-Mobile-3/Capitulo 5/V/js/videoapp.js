...
function cargarListaDeReproduccion(lista) {
    ...
    ldr.onreadystatechange = function() {
        if (ldr.readyState == 4 && ldr.status == 200) {
            ...
            for (let v of video) {
                listado += "<li class='collection-item avatar black-text' id='" + n + "'>"
                listado += "<i class='material-icons circle red darken-4 waves-effect waves-light " + v.boton + "' onclick='reproduzcoVideo(" + n + ")'>play_arrow</i> "
                listado += "<span class='title black-text'>" + v.titulo + "</span>"
                listado += "<p class='black-text'>" + v.desc + "</p>"
                listado += "</li>";
                n++;
            }
            listado += '</ul>';
